(ns ui.element.chooser.views
  ...
  (:require [#?(:clj clojure.core :cljs reagent.core) :refer [atom]]
            ...
            [ui.element.textfield.spec :as searchfield-spec]
...
(defn chooser
  ...
  (let [{:keys [params]}      (util/conform! ::spec/args args)
        ...
        searchfield-elem*       (clojure.core/atom nil)]
    ...
    (fn [& args]
      (let [{:keys [params]}          (util/conform! ::spec/args args)
            ...
            searchfield-params (merge (dissoc params :items)
                                      {:id        (util/slug id "searchfield")
                                       :model     query*
                                     ...
                                     ;; Hide collection and execute external on-blur
                                      ...
                                      #?(:cljs {:ref #(reset! searchfield-elem* (dom/dom-node %))})
                                      (if searchable
                                        {:label     label
